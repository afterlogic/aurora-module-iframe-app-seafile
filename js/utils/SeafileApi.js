...
module.exports = {
	getRepos: function (callback) {
		getSeafileResponse(`${Settings.Host}api2/repos`, callback);
	},

	getRepoDir: function ({ repoId, dirName = '', parentDir = '' }, callback) {
		if (dirName) {
			const p = encodeURI(`${parentDir}${dirName}`);
			getSeafileResponse(`${Settings.Host}api2/repos/${repoId}/dir?p=${p}`, callback);
		} else {
			getSeafileResponse(`${Settings.Host}api2/repos/${repoId}/dir`, callback);
		}
	},

	createDir: function ({ repoId, dirName, parentDir }, callback) {
		const p = encodeURI(`${parentDir}${dirName}`);
		getSeafileResponse(`${Settings.Host}api2/repos/${repoId}/dir/?p=${p}`, callback, { operation: 'mkdir' });
	},

	getRepoData: function ({ repoId }, callback) {
		getSeafileResponse(`${Settings.Host}api/v2.1/repos/${repoId}/`, callback);
	},

	applyPassword: function ({ repoId, password }, callback) {
		getSeafileResponse(`${Settings.Host}api2/repos/${repoId}/`, callback, { password });
	},
		...
		const parameters = {
			...
			Files: files.map(file => {
				return {
					Name: file.name,
					Hash: file.id,
					Link: `${Settings.Host}api2/repos/${repoId}/file/?p=${parentDir}${file.name}`
				};
			})
		};
	...
	saveToSeafile: function ({ accountId, hashes, repoId, dirName }) {
		const
			p = encodeURI(dirName),
			url = `${Settings.Host}api2/repos/${repoId}/upload-link/?p=${p}`
		;
